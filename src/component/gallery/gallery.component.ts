import { ChangeDetectionStrategy, Component, OnDestroy, ElementRef, Renderer2 } from '@angular/core';
...
@Component({
  ...
  template: `<gallery-main *ngIf="state | async as st" (play)="play($event)"
          [state]="st"  (play)="play($event)" (next)="next($event)" (prev)="prev($event)"
          [config]="gallery.config"></gallery-main>`
})
